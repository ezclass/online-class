<div>
    <div class="top h-64 w-full overflow-hidden relative">
        <img src="{{ Storage::disk('do')->url('dashboard/dashboard.jpg') }}"
    ...
    <div class="px-6 md:px-32 flex justify-between lg:flex-row flex-col">
        <div class="flex lg:flex-row flex-col">
            <div class="">
                <img src="{{ Storage::disk('do')->url('avatar/' . $teacher->avatar) }}" alt=""
                    class="w-36 h-36 bg-cover rounded-full bg-center absolute transform -translate-y-1/2 ring-4 ring-white">
            </div>
    ...
    <div class="holder mx-auto w-11/12 grid sm:grid-cols-1 md:grid-cols-3 lg:grid-cols-4">
        ...
        @empty
            ...
            <div class="bg-yellow-100 border border-yellow-400 text-yellow-700 p-3 rounded relative my-6 w-full shadow"
                role="alert">
                <span class="block sm:inline text-yellow-700">No classes yet, Click <a
                        href="{{ route('create.program.viwe') }}"><u>Create a new class</u></a></span>
            </div>
            @endrole
            @role('student')
            <div class="bg-yellow-100 border border-yellow-400 text-yellow-700 p-3 rounded relative my-6 w-full shadow"
