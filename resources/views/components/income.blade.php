    ...
    <div class='shadow rounded-md bg-blue-100 w-full  h-64'>
        <div class=' items-center px-5 py-3'>
            <div class='mx-3'>
                ...
                <div class="mt-2 text-xl">Amount to be paid (Total - 10%) = Rs.{{$payments->sum('amount') - ($payments->sum('amount') / 100 * 10 )}}</div>
                <div class="mt-2 text-xl">Amount charged (10%) = {{ ($payments->sum('amount') / 100 * 10 )}}</div>
                <div class="mt-2 text-xl">Payhere charged (3.9%) = {{ ($payments->sum('amount') / 100 * 10  / 100 * 3.9 )}}</div>
