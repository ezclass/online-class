    ...
    <form action="{{route('paid.save')}}" method="POST">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-4">
            ...
            <div>
                ...
                <input type="text" name="amount" value="{{ $payments->sum('amount') / 100 * 90 + $homepayments->sum('amount') / 100 * 80}}" class="border border-gray-400 block py-2 px-4 w-full rounded focus:outline-non focus:border-teal-500" placeholder="Your Class Name" required>
            </div>
            <div>
