    ...
    <div class="dark:bg-gray-900 py-6 flex flex-col justify-center sm:py-12">
        <div class="grid lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-4 px-4">
            ...
            <div class="bg-gray-100 border-indigo-600 dark:bg-gray-800 bg-opacity-95 border-opacity-60 | p-4 border-solid rounded-3xl border-2 | justify-around cursor-pointer | hover:bg-indigo-200 dark:hover:bg-indigo-600 hover:border-transparent | transition-colors duration-500">
                <a href="{{ route('public.dashboard', $ranking->teacher->getRouteKey()) }}">
                    <img class="inline-block h-14 w-14 rounded-full ring-2 ring-white" src="{{Storage::disk('do')->url('avatar/'.$ranking->teacher->avatar)}}" alt="" />
                    <div class="flex flex-col justify-center">
                        <p class="mt-2 text-gray-900 dark:text-gray-300 font-semibold hover:underline">{{$ranking->teacher->name}}</p>
                        <p class="text-black dark:text-gray-100 text-justify font-semibold">{{$ranking->teacher->getRouteKey()}}</p>
                    </div>
                </a>
                <form action="{{route('remove.ranking', $ranking->getRouteKey())}}" method="POST">
                    @csrf
                    <input type="hidden" name="rank_user" value="{{$ranking->teacher->getRouteKey()}}">
            ...
            <div class="bg-yellow-100 border border-yellow-400 text-yellow-700 p-3 rounded relative my-6 w-full shadow" role="alert">
                ...
                <span class="block sm:inline">You are not ranking yet</span>
            </div>
            @endforelse
        </div>
    </div>
